...
(function ($, window, document, undefined) {
    ...
    /*    data-toggle="maincontext" data-target="#maincontext-menu"*/
    //结构模板
    BaseTab.prototype.template = {
        ul_nav: '<ul id="myTab"  class="nav nav-tabs"></ul>',
        ul_li: '<li id="{0}li" ><a href="#{0}" data-toggle="tab"><span>{1}</span></a></li>',
    ...
    BaseTab.prototype.addTab = function (obj) {
        var self = this;
        //如果已经打开了，就不要重复打开，直接显示
        if (this.$element.find(".nav-tabs a[href='#" + obj.id + "']").length > 0) {
        ...
        this.$element.find(".nav-tabs li a[href='#" + obj.id + "']").mousedown(function (e) {
            mainNowTab =obj.id;
            //右键为3
            if (3 == e.which) {
                var menu = document.getElementById("miantabmenu");
                var ev = ev || event;
