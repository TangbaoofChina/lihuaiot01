...
/**
 * @ClassName Lhty02p1Util
 * @Description TODO
 * @Author tangbao
 * @Date 2020-06-21 10:23
 * @Version 1.0
 **/
public class Lhty02p1Util {
    //数据库查询的数据，按照设备进行分类
    public static Map<String, List<Lhty02p1DMHis>> splitMsgByIds(List<Lhty02p1DMHis> dMList, String[] sDeviceIds) {
        Map<String, List<Lhty02p1DMHis>> dmMap = new HashMap<>();
        for (String deviceId : sDeviceIds
        ) {
            List<Lhty02p1DMHis> dmList01 = new ArrayList<>();
            for (Lhty02p1DMHis msg : dMList
            ) {
    ...
    public static List<String> getDate01List(Map<String, List<Lhty02p1DMHis>> dmMapByDate) {
        List<String> sDateList = new ArrayList<>();
        for (Map.Entry<String, List<Lhty02p1DMHis>> entry : dmMapByDate.entrySet()) {
            for (Lhty02p1DMHis dm : entry.getValue()
            ) {
                //需要遍历所有设备的所有日期，防止有遗漏
