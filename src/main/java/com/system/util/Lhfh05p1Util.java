 ...
 * @Version 1.0
 **/
public class Lhfh05p1Util {
    //数据库查询的数据，按照设备进行分类
    public static Map<String, List<Lhfh05p1DMHis>> splitMsgByIds(List<Lhfh05p1DMHis> dMList, String[] sDeviceIds) {
        Map<String, List<Lhfh05p1DMHis>> dmMap = new HashMap<>();
        for (String deviceId : sDeviceIds
        ) {
            List<Lhfh05p1DMHis> dmList01 = new ArrayList<>();
            for (Lhfh05p1DMHis msg : dMList
            ) {
    ...
    public static List<String> getDate01List(Map<String, List<Lhfh05p1DMHis>> dmMapByDate) {
        List<String> sDateList = new ArrayList<>();
        for (Map.Entry<String, List<Lhfh05p1DMHis>> entry : dmMapByDate.entrySet()) {
            for (Lhfh05p1DMHis dm : entry.getValue()
            ) {
                //需要遍历所有设备的所有日期，防止有遗漏
