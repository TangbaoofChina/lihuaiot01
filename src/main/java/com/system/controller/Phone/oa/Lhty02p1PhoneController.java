...
@RequestMapping("/phone/lhty02p1")
public class Lhty02p1PhoneController extends BaseController {
    ...
    public String selectOrg(String userId) throws Exception {
        ...
        if (RoleInfoListUtil.checkIsAdmin(roleInfoList)) {
            List<RoleInfo> roleInfoListAdmin = roleInfoService.selectRoleInfoByRoleName("LHTY02p1");
            oTL = phoneBootStrapTreeNodeService.selectOrgTreeNodeInfoByRoleId("LHTY02p1", roleInfoListAdmin);
        } else {
            oTL = phoneBootStrapTreeNodeService.selectOrgTreeNodeInfoByRoleId("LHTY02p1", roleInfoList);
        }
    ...
    public String selectRealDeviceInfoSummary01(String userId, String orgId) throws Exception {
        List<Lhty02p1DM> dmList = getRealDMByUserIdAndOrgId(userId, orgId);
        String jsonString = JSON.toJSONString(ResponseUtil.setResponsFaild("未获取到有效数据"));
        if (dmList.size() > 0) {
     ...
     * @throws Exception
     */
    private List<Lhty02p1DM> getRealDMByUserIdAndOrgId(String userId, String orgId) throws Exception {
    ...
    private List<PhoneRealDeviceInfo> getPhoneRealDeviceInfoSummary(List<Lhty02p1DM> dmList) {
        List<PhoneRealDeviceInfo> phoneRealDeviceInfoList = new ArrayList<PhoneRealDeviceInfo>();
        for (Lhty02p1DM dm : dmList
        ) {
    ...
    private PhoneRealDeviceInfo getOneRealDeviceInfoSummary(Lhty02p1DM dm) {
