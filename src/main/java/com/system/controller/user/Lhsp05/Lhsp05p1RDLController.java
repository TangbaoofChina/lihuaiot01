...
public class Lhsp05p1RDLController {
    @Autowired
    private Lhsp05p1DMService dmService;

    @RequestMapping(value = "selectLhsp05p1ByORGId", method = {RequestMethod.POST}, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public String selectByORGId(String sORGId) throws Exception {
        String jsonString = "[]";
        if (sORGId != null) {
            List<Lhsp05p1DM> dmList = selectDMList(sORGId);
            if (dmList.size() > 0)
    ...
    @RequestMapping(value = "selectLhsp05p1ByDeviceId", method = {RequestMethod.POST}, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public String selectFeedC411ByDeviceId(String sDeviceId) throws Exception {
        String jsonString = "[]";
        if (sDeviceId != null) {
            Lhsp05p1DM dm = dmService.selectByDeviceId(sDeviceId);
            if (dm != null)
    ...
    @RequestMapping(value = "exportLhsp05p1DeviceList", method = RequestMethod.GET)
    public void exportFeedC411DeviceList(String sORGId,
                                         HttpServletRequest request,
                                         HttpServletResponse response) throws Exception {
        String fileName = "realLhsp05p1devicelist.xlsx";
    ...
    private List<Lhsp05p1DM> selectDMList(String sORGId) throws Exception {
        List<Lhsp05p1DM> dmList = new ArrayList<>();
        //获取用户角色
